...
set myFolder=run_1_slab_LENS_r-triple_3030_%myDate%_time_%myTime%
echo %myFolder%
...
set domain=200kmx200kmx20km_LENS_4.3dm
...
set fixed_flags=-M -load_rhino %domain% -allow_slaves_as_intersection_masters -solver_max_cycles 1000 -bc Europa_BCT -solver_pardiso
...
set material_flags=-ym 10e9 -pr 0.33 -uts 2e6 -kic 108e3 -beta_n 0 -beta_f 0
rem

set refine_flags=-refine 1 -fixed_volume_refine 5e3 -fixed_edge_refine 5e3
set mesh_flags=-meshing_flagsn "ic_geo_set_family_params INTLENS_G_S emax 5000 internal_wall 1" -meshing_flagsn "ic_geo_set_family_params MATRIX_15 emax 500"
...
set runtime_flags=-nr_of_tips 20 -min_ext 0 -max_it 4 -nostress -no_exit -no_mapping -noprint_tip_info
set nuc_flag=-ft 1e4 -nucleation -nuc_flaw_size 1e3 -nucleation_max 40 -nuc_abs_spacing 10e3 -nuc_step_spacing_multiplier 0.95 -nucleation_max_multiplier 1
rem

cmd /k %run%.exe %fixed_flags% %growth_flags% %box_flags% %misc_flags% %material_flags% %refine_flags% %slab_flags% %runtime_flags% %frac_flags% %mesh_flags% %data_flags%
rem %run%.exe %fixed_flags% %growth_flags% %box_flags% %misc_flags% %material_flags% %refine_flags% %slab_flags% %runtime_flags% %nuc_flag% %speed_flags%


rem %europa_layer_flags% %europa_region_flags% -start_time 1e2 %data_flags% -nucleation_flaw_size_multiplier 0.95
rem -fixed-extension-w -skip_sifs_and_growth -bcp -1.e-5 -friction -uzawa_outer 2 -uzawa_inner 2 -rfs
